...
import { query, getConnection } from '@/lib/database';
...
export async function GET(request: NextRequest) {
  ...
  const batchSize = parseInt(searchParams.get('batchSize') || '3'); // Default batch size of 3
  ...
  logSync(`Starting ${syncType} batch sync with batch size ${batchSize}...`);
  ...
  try {
    ...
    logSync(`Processing ${batches.length} batches of ${batchSize} cars each`);
    
    for (let i = 0; i < batches.length; i++) {
      const batch = batches[i];
      logSync(`Processing batch ${i + 1}/${batches.length} (${batch.length} cars)...`);
      ...
      try {
        ...
        // Get existing car IDs for this batch
        const autobazarIds = batch.map(ad => ad.idAdvertisement);
        const placeholders = autobazarIds.map(() => '?').join(',');
        const [existingCars] = await connection.execute(
          `SELECT autobazar_id FROM cars WHERE autobazar_id IN (${placeholders}) AND status != "deleted"`,
          autobazarIds
        ) as [Array<{ autobazar_id: string }>, unknown];
        ...
        logError(`Error processing batch ${i + 1}:`, error);
        result.errors += batch.length;
      } finally {
        // Always close the connection for this batch
        if (connection) {
          try {
            await connection.end();
          } catch (error) {
            logError('Error closing batch connection:', error);
          }
        }
      }
      ...
      if (i < batches.length - 1) {
        logSync('Waiting 3 seconds before next batch...');
    ...
    await query(
      ...
      [
        ...
        `Batch sync completed: ${result.added} added, ${result.updated} updated, ${result.deleted} deleted, ${result.errors} errors`,
        result.added,
        result.updated,
        result.deleted,
        result.skipped,
    ...
    logSuccess(`Batch sync completed: ${result.added} added, ${result.updated} updated, ${result.deleted} deleted, ${result.errors} errors`);
    
    // Send email notification if there were errors
    if (result.errors > 0) {
      await sendSyncNotification(
        'Batch sync completed with errors',
        `Batch sync completed with ${result.errors} errors. Added: ${result.added}, Updated: ${result.updated}, Deleted: ${result.deleted}`
      );
    }
    ...
    await query(
      'INSERT INTO sync_logs (sync_type, status, message, cars_added, cars_updated, cars_deleted, cars_skipped, errors_count, execution_time_seconds) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [syncType, 'error', `Batch sync failed: ${error}`, 0, 0, 0, 0, 1, result.executionTime / 1000]
    );
    
    // Send error notification
    await sendSyncNotification('Batch sync failed', `Batch sync failed with error: ${error}`);
    
    return NextResponse.json({ error: 'Batch sync failed', details: error }, { status: 500 });
  }
}
